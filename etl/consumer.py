...
class WindowedAggregator:
    ...
    def __init__(self):
        ...
        # In-memory window storage: {route_id: {bucket_start_ts: [delays]}}
    ...
    def _add_to_window(self, route_id: str, delay_seconds: int, event_ts: datetime) -> None:
        """
        ...
        self.windows[route_id][bucket_start].append(delay_seconds)
        logger.debug(f"Added delay {delay_seconds}s for route {route_id} to window {bucket_start}")
    
    def _calculate_metrics(self, delays: List[int]) -> Tuple[float, float, int]:
        """
        Calculate aggregation metrics for a list of delays.
        
        Args:
            delays: List of delay values in seconds
            
        Returns:
            Tuple of (avg_delay_seconds, ontime_percentage, anomaly_count)
        """
        if not delays:
            return 0.0, 0.0, 0
        ...
        return avg_delay, ontime_pct, anomalies
    ...
    def _flush_window(self, route_id: str, bucket_start: datetime) -> None:
        ...
        """
        delays = self.windows[route_id].pop(bucket_start, [])
        if not delays:
            return
        
        avg_delay, ontime_pct, anomalies = self._calculate_metrics(delays)
        ...
        logger.info(
            f"Flushed window {bucket_start} for route {route_id}: "
