...
const DelayChart = ({ data, routeName, direction }) => {
  ...
  const delayData = filteredData.map(item => 
    item.avg_delay_seconds !== null && item.avg_delay_seconds !== undefined
  ...
  const chartTitle = direction !== 'both' 
    ? `Delay (seconds) - ${direction.charAt(0).toUpperCase() + direction.slice(1)}`
    : 'Delay (seconds)';
